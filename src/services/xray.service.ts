...
export class XrayService {
  private static readonly XRAY_CONFIG_PATH = "/usr/local/etc/xray/config.json";
  private static readonly XRAY_CONFIG_BACKUP= "/usr/local/etc/xray/config.json";
  ...
  static async updateConfig(config: XrayConfig): Promise<string> {
    const backupPath = `${this.XRAY_CONFIG_BACKUP}.backup.${Date.now()}`;
